  ...
  return (
    <Stack direction="row" display="inline-grid">
      <SelectArrayInput
        ...
        choices={choices}
        onChange={({ target }) =>
          setCount(Array.isArray(target.value) ? target.value.length : 0)
        }
        optionText={(choise) => <TagChip tag={choise} count={count} />}
        source="keywords"
      />
      <Stack
        direction="row"
      ...
      >
        ...
        <Button
          ...
          disabled={!(choices.length > inputKeys)}
          sx={{ width: '45%', mb: 3 }}
          variant="contained"
          onClick={() =>
...
const CreateNewTagDialog = ({ open, setOpen, setChoices }) => {
  ...
  const handleSubmit = (event) => {
    event.preventDefault();
    setChoices((prevChoices) => {
      prevChoices.push({ id: prevChoices.length + 1, name: value });
      return prevChoices;
    });
    setValue('');
    setOpen(false);
    ...
    >
      ...
      <DialogContent>
        <TextField
          ...
          helperText={
            checkIsValid(value) ? 'Текст тэга' : 'Допустимо до 10 символов'
          }
          sx={{ m: 0.5 }}
          autoFocus
        />
      ...
      <DialogActions>
        <Button disabled={!checkIsValid(value)} onClick={handleSubmit}>
          Добавить
        </Button>
