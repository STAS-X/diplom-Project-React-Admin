  ...
  const [updateMany, { loading, error }] = useUpdateMany(
    ...
    { status },
    {
      ...
      onSuccess: () => {
        refresh();
        notify(
          status ? 'Задачи завершены успешно' : 'Задачи открыты для исполнения',
          { undoable: true }
        );
        unselectAll();
      },
      onError: (error) =>
        notify('Ошибка при обновлении задач!', { type: 'warning' }),
    }
  );
...
const MyTaskDataGrid = ({ userId, ...props }) => {
  ...
  React.useEffect(() => {
    ...
    return () => {};
  }, [taskRef.current, loading, isAppColorized]);
  ...
  return (
    <Datagrid
      ...
      rowStyle={isAppColorized ? taskRowStyle(authUser.uid) : () => {}}
      ref={taskRef}
    >
...
const MyCommentDataGrid = ({ userId, ...props }) => {
  ...
  React.useEffect(() => {
    ...
    return () => {};
  }, [commentRef.current, loading, isAppColorized]);
  ...
  return (
    <Datagrid
      isRowSelectable={(record) => true}
      rowStyle={isAppColorized ? commentRowStyle(authUser.uid) : () => {}}
      ref={commentRef}
    >
      <TextField label="Описание" sortable={false} source="description" />
      <DateField
...
export const UserTabbetShow = (props) => {
  ...
  if (pathname.slice(-4) === 'show') {
    ...
    props.history.location.pathname =
      props.history.location.pathname + (newTab ? newTab : '/user');
  }
