...
const ExecutorsListField = ({ executors: ids }) => {
  ...
  return (
    <>
      {users.map((user, ind) => (
        <Chip key={ind} label={user.name ? user.name : '-XXX-'} />
      ))}
    </>
  );
};
...
export const TaskTabbetShow = (props) => {
  const { data: task } = useGetOne('tasks', props.id);
  ...
  if (pathname && pathname.slice(-4) === 'show') {
    ...
    props.history.location.pathname =
      props.history.location.pathname + (newTab ? newTab : '/task');
  }
      ...
      >
        ...
        >
          ...
          <FunctionField
            ...
            source="keywords"
            render={(record) => (
              <KeywordsListField keywords={record.keywords} />
            )}
          />
          <FunctionField
            label="Завершена"
          ...
          <FunctionField
            ...
            source="commentable"
            render={(record) => (
              <span>{record.commentable ? 'Да' : 'Нет'}</span>
            )}
          />
        </Tab>
        <Tab
